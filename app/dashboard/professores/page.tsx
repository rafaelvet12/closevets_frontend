...
export default function ProfessoresPage() {
  ...
  const fetchProfessores = async () => {
    try {
      const res = await fetch(`http://127.0.0.1:8000/professores/`);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const url = isEditMode ? `http://127.0.0.1:8000/professores/${selectedProfessor?.id}` : `http://127.0.0.1:8000/professores/`;
  ...
  const handleDelete = (prof: Instructor) => {
    setConfirmModal({
      ...
      onConfirm: async () => {
        try {
          const res = await fetch(`http://127.0.0.1:8000/professores/${prof.id}`, { method: "DELETE" });
          if (res.ok) {
