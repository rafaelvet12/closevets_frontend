...
export default function AlunosPage() {
  ...
  const fetchAlunos = async () => {
    try {
      const res = await fetch("http://127.0.0.1:8000/alunos/");
  ...
  const openDetails = async (student: Student) => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:8000/matriculas/aluno/${student.id}`);
  ...
  const handleSaveStudent = async (e: React.FormEvent) => {
    ...
    try {
      const url = isEditMode ? `http://127.0.0.1:8000/alunos/${selectedStudent?.id}` : `http://127.0.0.1:8000/alunos/`;
  ...
  const handleDeactivate = (student: Student) => {
    setConfirmModal({
      ...
      onConfirm: async () => {
        try {
          const res = await fetch(`http://127.0.0.1:8000/alunos/${student.id}`, { method: "DELETE" });
          if (res.ok) { 
